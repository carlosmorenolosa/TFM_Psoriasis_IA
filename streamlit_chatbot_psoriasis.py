...
txt_formulario_url = (
    "https://iaenpsoriasis.pythonanywhere.com/static/juanjo_amoros_form.txt"
)
txt_tratamiento_url = (
    "https://iaenpsoriasis.pythonanywhere.com/static/juanjo_amoros_tratamiento.txt"
)
...
if txt_tratamiento_url:
    try:
        ...
        if resp.status_code == 200:
            ...
            st.warning(
                f"No se pudo descargar el tratamiento. Status: {resp.status_code}"
            )
    except Exception as e:
...
if st.button("Enviar"):
    if query.strip():  
        ...
        with st.spinner("Su respuesta se está generando..."):
            ...
            filter_conditions = {
                "$and": [{"texto": {"$exists": True}}, {"fuente": {"$exists": False}}]
            }
            ...
            textos_similares = [
                res["metadata"]["texto"] for res in resultados["matches"]
            ]
